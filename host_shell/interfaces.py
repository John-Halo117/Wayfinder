"""Typed Host Shell provider interfaces.
...
- Failure cases: disabled provider, invalid request, unavailable provider,
  delegated adapter failure, and invalid provider selection.
...
@dataclass(frozen=True)
class HostShellHealth:
    """Provider health signal for Host Shell interaction runtime."""
...
@dataclass(frozen=True)
class HostShellRequest:
    """Bounded Host Shell interaction request."""
...
@dataclass(frozen=True)
class HostShellResponse:
    """Bounded Host Shell interaction response."""
...
class HostShellProvider(Protocol):
    """Explicit provider contract for Host Shell runtime adapters."""
    ...
    def health(self) -> HostShellHealth:
        ...
        Inputs: provider configuration. Outputs: HostShellHealth.
        Runtime: bounded by provider timeout. Memory: bounded by provider caps.
        Failure: represented by HostShellHealth.failure.
    ...
    def send(self, request: HostShellRequest) -> HostShellResponse:
        """Send one Host Shell interaction request.
        ...
        Failure: represented by HostShellResponse.failure.
        Deterministic: validation deterministic; network depends on provider.
        """


def validate_host_shell_request(request: HostShellRequest) -> Failure | None:
    """Validate Host Shell request shape without calling a provider.

    Inputs: HostShellRequest. Outputs: optional Failure.
    ...
    if not prompt:
        return Failure.build("HOST_SHELL_PROMPT_INVALID", "prompt is required")
    if len(prompt) > MAX_PROMPT_CHARS:
        return Failure.build("HOST_SHELL_PROMPT_TOO_LARGE", "prompt exceeds maximum length")
    session_id = request.session_id.strip()
    if not session_id:
        return Failure.build("HOST_SHELL_SESSION_INVALID", "session_id is required")
    if len(session_id) > MAX_SESSION_ID_LENGTH:
        return Failure.build("HOST_SHELL_SESSION_INVALID", "session_id exceeds maximum length")
    return None
