...
| Reality Identity (RID) | Implement the constitutional identity model and continuity anchor. | Active | Identity Service, Asset Model | RID-M-003 RID Model | Identity Service / Constitution | RID is stable, representation-independent, evidence-bound, and consumed by ARK. | `docs/reality-identity-program.md` |
| Universal Asset Ingestion | Plan the reusable ingestion foundation for every asset type. | Planned | RID Program, Asset Model, ARK boundary | UAI-M-001 Pipeline Contracts after RID-M-003 | ARK / Platform | Universal ingestion replaces media-specific pipeline ownership with adapters. | `docs/universal-asset-ingestion-program.md` |
