# Promotion Report: Permission Contract

## Summary

Concept promoted: Permission
...
| `contracts/permissions/README.md` | New or normalized canonical language home. | Created or updated. |

## Dependency Graph

```mermaid
flowchart TD
  constitution["constitution/"] --> contract["contracts/permissions/"]
...
```mermaid
flowchart TD
  concept["Permission"] --> owner["contracts/permissions/"]
  owner --> consumers["documented consumers"]
```

## Canonical Owner Verification

Permission is shared architectural language. Its canonical contract home is
`contracts/permissions/`. Runtime behavior remains with future implementation owners and
...
1. Create or normalize `contracts/permissions/README.md` as language-only contract documentation.
...
Created or updated `contracts/permissions/README.md`.
...
Revert `contracts/permissions/README.md` and this promotion's governance updates.
