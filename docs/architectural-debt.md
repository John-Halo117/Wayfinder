...
| DEBT-003 | Duplicate ownership | Event contracts and event bus behavior appear in several ARK modules. | ARK legacy event schemas, GSB, NATS, WAL, transport | `contracts/events/`, `services/event-bus/` | High | Event contract and bus adapter proof passes. | Open |
...
| DEBT-005 | Duplicate ownership | Identity/subject concepts appear in ARK subjects and policy rules. | ARK legacy subjects and identity rules | `services/identity/`, `contracts/identities/` | High | Identity lookup and alias semantics are proven. | Open |
| DEBT-006 | Deferred promotion | Policy service is identified but not scaffolded. | ARK policy files, MCP policy, epistemic policy | `services/policy/`, `contracts/policies/` | High | Policy inventory and purity proof completed. | Open |
| DEBT-007 | Deferred promotion | Configuration service is identified but not scaffolded. | ARK config/env/manifests, Jarvis env, Foundry config | `services/configuration/` | Medium | Config schema and redaction proof completed. | Open |
...
| DEBT-005 | Identity/subjects duplication | Open | Reduced 2026-06-27; canonical owner promoted | ARK subject modules and identity rules still local |
| DEBT-006 | Policy service not scaffolded | Open | Resolved 2026-06-27; service scaffold exists | Implementation adapters and compatibility tests deferred |
| DEBT-007 | Configuration service not scaffolded | Open | Resolved 2026-06-27; service scaffold exists | Loader extraction and precedence tests deferred |

## Phase 4 Identity Debt Update

| Debt ID | Concept | Previous Status | Phase 4 Status | Remaining Debt |
| --- | --- | --- | --- | --- |
| DEBT-005 | Identity/subjects duplication | Reduced by Wave 2 ownership promotion | Further reduced 2026-06-27; reusable identity implementation promoted | ARK subject routing is reclassified as Event Bus debt; engine adapters not yet wired |
