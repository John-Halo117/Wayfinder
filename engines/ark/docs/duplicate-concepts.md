...
| Identity and subject routing | `ark/subjects.py`, `internal/subjects/subjects.go`, `policy/ark_identity_rules.json` | `services/identity/` for identity; `services/event-bus/` for routing subjects | Extract contract/service adapters, then replace ARK-local duplicate with references. |
