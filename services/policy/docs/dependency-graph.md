# Policy Service Dependency Graph

```mermaid
graph TD
  constitution["constitution/"] --> contracts["contracts/"]
  contracts --> service["services/policy/"]
...
`services/policy/` must not depend on engines, domains, internal applications, external integrations, or operations.
